...
def school_identity_card(name: str, meta: str) -> None:
    st.markdown(
        ...
        <div class='school-identity-card'>
            ...
            <div class='school-identity-name'>{name}</div>
            <div class='school-identity-meta'>{meta}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def summary_box(lines: List[str]) -> None:
    items = "".join([
        f"<li style='padding:0.12rem 0; border-bottom:1px solid #f0f5fb;'>{x}</li>"
        for x in lines
    ])
    st.markdown(
        f"""
...
def page_application_eval(base_df: pd.DataFrame, selected_df: pd.DataFrame) -> None:
    ...
    if "계획서 요약" in sub:
        ...
        with l:
            st.markdown(
                ...
                <div class='section-card'>
                    ...
                    <div class='section-title'>{selected_school}</div>
                    <div class='section-desc'>
                    핵심 문제: {row.get('first_choice_area_norm', '학업지원')} 영역 수요가 확인됩니다.<br>
                    대상 학생: {row.get('school_level_group', '')} 학생군 중심<br>
                    사업 유형: {row.get('first_choice_area_norm', '')}형<br>
        ...
        with r:
            ...
            st.markdown(f"<div class='logic-box'><div class='logic-title'>권장 예산 산식</div><div class='logic-desc'>권장예산 = 기준단가 × 학생규모계수 × 취약도계수 × 운영난도계수<br><br>현재 학교급: {row.get('school_level_group', '')}<br>핵심 권장 영역: {', '.join(LEVEL_FOCUS.get(row.get('school_level_group', '중등'), []))}<br>최종배분점수: {row.get('final_allocation_score', 0):.1f}점</div></div>", unsafe_allow_html=True)
...
def page_school_report(base_df: pd.DataFrame, selected_df: pd.DataFrame, hold_df: pd.DataFrame) -> None:
    ...
    if "점수 풀이" in sub:
        ...
        summary_box([
            f"{row['school_display']}은(는) {row['school_level_group']} 내부 비교 기준에서 {row.get('final_allocation_score', 0):.1f}점으로 검토 대상에 올랐습니다.",
